...
async function chart() {
  try {
    ...
    console.log(
      "An error occured in orders count function chart",
      error.message
    );
  }
}
...
async function monthgraph() {
  try {
    ...
    console.log(
      "Error retrieving orders in monthgraph function:",
      error.message
    );
    return { labels: [], count: [] };
  }
...
async function yeargraph() {
  try {
    ...
    console.log(
      "Error retrieving orders in yeargraph function:",
      error.message
    );
    return { labels: [], count: [] };
  }
...
dashController.getDashboard = async (req, res, next) => {
  try {
    ...
    const ordersPie     = (await chart())       || { pending: 0, shipped: 0, delivered: 0, cancelled: 0 };
    const ordersGraph    = (await monthgraph())   || { labels: [], count: [] };
    const ordersYearGraph = (await yeargraph())   || { labels: [], count: [] };
